...
# Jillian created grids for HBLL N and S that have over water area and area weighted depth. See here: https://github.com/pbs-assess/gfdata/blob/85f4355ab7cba133674f425e3892f80e74894dfa/data-raw/survey_blocks.R#L80
...
soggrid3 |> 
  drop_na(area_orig) |> 
  ggplot() +
  geom_point(aes(area_orig, area_km2)) #differences between Jillian's overwater calc (area_orig) and what's completed here. 
...
ggplot() +
   ...
   geom_sf(data = bdepths_int, aes(colour = west_coast_dem))

bdepths2 <- bdepths_int |>
